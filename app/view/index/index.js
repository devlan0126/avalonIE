...
var vm = avalon.define({
  ...
  getParams: function () {
    ...
    mainOprnCodeList = mainOprnCodeList.substring(
      0,
      mainOprnCodeList.length - 1
    );
      ...
      success: function success(res) {
        if (res.code === 200) {
          if (res.data.code === 200) {
            ...
            that.processInfo = {
              real_short_amount: that.setlInfo.lowCbspCritVal || 0,
              drg_limit: that.setlInfo.highCbspCritVal || 0,
              total_amount: that.setlInfo.medFeeAmt || 0,
            };
            var progressId =
              that.clrWay === "50" ? "dipProgress" : "drgProgress";
            onProgressHandle(
              that.processInfo,
  ...
  onZlChange: function ($event, val) {
    ...
    window.zlDiagnose =
      typeof window.zlDiagnose === "boolean" ? window.zlDiagnose : false;
    window.zlOperation =
      typeof window.zlOperation === "boolean" ? window.zlOperation : false;
    this.fzzlList = filterZlList(
      this.fzzlAllList,
      window.zlDiagnose,
      window.zlOperation,
      this.dataForm.mainDiagCode,
      this.dataForm.mainOprnCodeList
    );
  },
  config: {
    isShow: false,
...
vm.$watch("onReady", function (v) {
  ...
  getConfigByDeployMode(function (data) {
    ...
    if (that.clrWay === "40") {
      that.warnTips = "医疗费用为DRG病组均费的";
    } else {
      that.warnTips = "医疗费用为DIP病种均费的";
    }
      ...
      function (data, isError) {
        ...
        for (var i = 0; i < zyssjczList.length; i++) {
          if (zyssjczList[i].ssjczbm || zyssjczList[i].ssjczmc) {
            zyssTemp.push(
              zyssjczList[i].ssjczbm + "," + zyssjczList[i].ssjczmc
            );
        ...
        if (!isError) {
          ...
          that.warnMsgList = res.warnMsgList || [];
          that.processInfo = {
            real_short_amount: that.setlInfo.lowCbspCritVal || 0,
            drg_limit: that.setlInfo.highCbspCritVal || 0,
            total_amount: that.setlInfo.medFeeAmt || 0,
  ...
  getDiagnoseList();
  resetBottomHeight();
  getDictSimFields(function (data) {
    var simplifiedMap = {};
    for (var i = 0; i < data.length; i++) {
      simplifiedMap[data[i].dictValue] = data[i].dictLabel;
    }
    that.simplifiedMap = simplifiedMap;
  });
});
...
function resetBottomHeight() {
  ...
  var h = bottomHeight - $tabsUl.offsetHeight;
...
function getConfigByDeployMode(callback) {
  $.ajax({
    url: "/portal/system/auth/pop/getConfigByDeployMode",
    type: "GET",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
...
function getDictSimFields(callback) {
  $.ajax({
    url: "/portal/system/dict/data/list?pageNum=1&pageSize=25&dictType=grp_sim_field",
    type: "GET",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
...
function getPageInfo(serialNo, callback, that) {
  $.ajax({
    ...
    dataType: "json",
    success: function success(res) {
      var previewParam = getUrlParam("preview");
      if (res.code === 200) {
        if (res.res.code === 200) {
          callback(res);
        } else {
        ...
        if (res.title) {
          that.pageTitle = res.title || "医院DRG/DIP数据精细化治理";
          document.title = res.title || "医院DRG/DIP数据精细化治理";
        }
        if (!previewParam) {
          alert(res.msg);
        }
...
function encodeGroupInfo(groupInfo) {
  if (groupInfo && groupInfo.setlInfo) {
    ...
    return {
      ...
      feeRatio: setlInfo.feeRatio ? (setlInfo.feeRatio || 0) + "%" : "-", // 费用比例
      ...
      medFeeAmt: setlInfo.cbspActVal, // 总费用
      lowCbspCritVal: setlInfo.lowCbspCritVal, // 低限金额
      highCbspCritVal: setlInfo.highCbspCritVal, // 高限金额
  ...
  return {
    ...
    medFeeAmt: 0, // 总费用
    lowCbspCritVal: 0, // 低限金额
    highCbspCritVal: 0, // 高限金额
  };
}

function onProgressHandle(drgGroupInfo, dom) {
  var lowCutoff = parseFloat(
    drgGroupInfo.real_short_amount ? drgGroupInfo.real_short_amount : 0.0
  );
  var highCutoff = parseFloat(
    drgGroupInfo.drg_limit ? drgGroupInfo.drg_limit : 0.0
  );
  var hispitalTotalAvg = (highCutoff - lowCutoff) / 2;
  var cost = parseFloat(
    drgGroupInfo.total_amount ? drgGroupInfo.total_amount : 0.0
  );
